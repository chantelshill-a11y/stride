...
// /api/run is now used only for replay mode (deterministic playback, no
// Anthropic calls) and the export-calendar path. Both finish well under
// Netlify Lambda's 30s streaming cap, so plain Node.js runtime is fine.
// Live mode runs in the browser via components/lib/orchestrate-*.ts,
// which calls /api/agent once per agent (each <30s).
